const CACHE='rmc-gestion-v9-ocr-columns';
...
const OCR_BOOT='<script>(function(){var r=document.getElementById("receiptFile");if(r){var c=r.cloneNode(true);r.parentNode.replaceChild(c,r);}var s=document.createElement("script");s.src="./ocr-v2.js?v=4";document.body.appendChild(s);})();<\/script>';
...
self.addEventListener('fetch',e=>{
 ...
 if(u.origin===location.origin && (u.pathname.endsWith('/RMC-GESTOR/')||u.pathname.endsWith('/index.html'))){
  e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{let t=await r.text();if(!t.includes('ocr-v2.js?v=4'))t=t.replace('</body>',OCR_BOOT+'</body>');const h=new Headers(r.headers);h.set('content-type','text/html; charset=utf-8');h.set('cache-control','no-store');return new Response(t,{status:r.status,statusText:r.statusText,headers:h})}));return;
 }
